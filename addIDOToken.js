...
const w3 = new Web3(specs.testNetUrl);
...
async function run() {
  ...
  data = newTokenContract.instance.methods
    ...
    .encodeABI();
  let receipt = await ethTransact(data, newTokenContract.instance.options.address, nonce);
  nonce += 1;
  data = idoplatformContract.instance.methods
    .addIDOToken(newTokenContract.instance.options.address)
    .encodeABI();
  receipt = await ethTransact(data, idoplatformContract.instance.options.address, nonce);
}
